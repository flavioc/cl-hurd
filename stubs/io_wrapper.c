		...
		vm_size_t *amount)
{
	...
	return write_routine(io_object, data, dataCnt,
			offset, amount);
}
		...
		vm_size_t amount)
{
	...
	return read_routine(io_object, data, dataCnt,
			offset, amount);
}

/* io seek */

typedef kern_return_t (*io_seek_type)(io_t,
		loff_t, int, loff_t *);
		...
		loff_t *newp)
{
	...
	return seek_routine(io_object, offset, whence, newp);
}
...
typedef kern_return_t (*io_set_all_openmodes_type)(io_t,
		int);
		...
		int *bits)
{
	...
	io_get_openmodes_type get_openmodes_routine =
		routines[IO_GET_OPENMODES];
		...
		mach_msg_type_name_t *icky_async_id_portPoly)
{
	...
	return get_icky_async_id_routine(io_object,
			icky_async_id_port, icky_async_id_portPoly);
}
...
typedef kern_return_t (*io_reauthenticate_type)(io_t,
		mach_port_t);
		...
		mach_port_t rendezvous2)
{
	...
	io_reauthenticate_type reauthenticate_routine =
		routines[IO_REAUTHENTICATE];
		...
		mach_msg_type_number_t gidsCnt)
{
	...
	io_restrict_auth_type restrict_auth_routine =
		routines[IO_RESTRICT_AUTH];
		...
		int *server_edit_level)
{
	...
	io_server_version_type server_version_routine =
		routines[IO_SERVER_VERSION];
		...
		mach_msg_type_name_t *memobjPoly)
{
	...
	io_map_cntl_type map_cntl_routine =
		routines[IO_MAP_CNTL];

	return map_cntl_routine(io_object, memobj,
			memobjPoly);
}
...
lisp_io_get_conch(io_t io_object)
{
	...
	io_get_conch_type get_conch_routine =
		routines[IO_GET_CONCH];
...
lisp_io_release_conch(io_t io_object)
{
	...
	io_release_conch_type release_conch_routine =
		routines[IO_RELEASE_CONCH];
...
lisp_io_eofnotify(io_t io_object)
{
	...
	io_eofnotify_type eofnotify_routine =
		routines[IO_EOFNOTIFY];
...
typedef kern_return_t (*io_prenotify_type)(io_t,
		...
		vm_offset_t write_end)
{
	...
	io_prenotify_type prenotify_routine =
		routines[IO_PRENOTIFY];

	return prenotify_routine(io_object,
			write_start, write_end);
}

/* io postnotify */

typedef kern_return_t (*io_postnotify_type)(io_t,
		vm_offset_t, vm_offset_t);
		...
		vm_offset_t write_end)
{
	...
	io_postnotify_type postnotify_routine =
		routines[IO_POSTNOTIFY];

	return postnotify_routine(io_object,
			write_start, write_end);
}
...
lisp_io_readnotify(io_t io_object)
{
	...
	io_readnotify_type readnotify_routine =
		routines[IO_READNOTIFY];
...
lisp_io_readsleep(io_t io_object)
{
	...
	io_readsleep_type readsleep_routine =
		routines[IO_READSLEEP];
...
typedef kern_return_t (*io_pathconf_type)(io_t,
		int, int *);
		...
		int *value)
{
	...
	io_pathconf_type pathconf_routine =
		routines[IO_PATHCONF];
		...
		ino64_t *fileno)
{
	...
	io_identity_type identity_routine =
		routines[IO_IDENTITY];

	return identity_routine(io_object, idport,
			idportPoly, fsidport, fsidportPoly,
			fileno);
}
