...
(defun helper-fetch-root (dotdot fetch-root-callback node-port uid gid path)
  (with-foreign-pointer (control (foreign-type-size 'port))
		(let ((return-code (%helper-fetch-root dotdot fetch-root-callback
                                           node-port uid gid path (+ 1 (length path)) control)))
