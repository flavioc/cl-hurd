		 ...
		 (with-foreign-pointer (extension (foreign-type-size 'vm-address))
	       ...
	       (with-foreign-pointer (dirent-ptr this-size)
				...
				;	   +name-offset+ dirent-name (+ +name-offset+ namelen) this-size)
				    ...
				    ; (warn "old pointer ~s~%" ptr)
				     (incf-pointer ptr this-size)))
				     ;(warn "pointer now to ~s~%" ptr)))
	  when (= cnt total)
	  return (progn
		   ;(warn "returning count ~s ptr ~s size ~s~%" (1+ cnt) ptr size)
		   (values (1+ cnt) ptr size)))))

;(setf *trace-output* *error-output*)
;(trace %dir-readdir-count)

(defun %dir-readdir (dir-protid bufsiz entry nentries existing-entries)
  ;(warn "%dir-readdir ~s ~s ~s ~s ~s ~%" dir-protid bufsiz entry nentries existing-entries)
...
(def-fs-interface :dir-readdir ((port port)
   ...
   (with-lookup dir-protid port
      (block dir-readdir
	     (unless (is (get-open-flags dir-protid) 'read)
		 ...
		 (when (null data-ptr)
		   (setf (mem-ref amount :int) 0)
		   (return-from dir-readdir t))
		 ;(warn "count: ~s ; data: ~s len: ~s ~%" cnt data-ptr len)
